                            ...
                            if (isset($_GET['search_date'])) { 
                                ...
                                <table id="example"  class="table table-centered table-striped table-bordered mb-0 toggle-circle" >
                                    ...
                                    <tbody>
                                        ...
                                        $t_discount = 0;
                                        $sr_no = 1;
                                        $fetch_data = "SELECT * FROM ssh_p_dialysis JOIN ssh_p_reg ON ssh_p_dialysis.P_ID = ssh_p_reg.P_ID  where ssh_p_dialysis.date BETWEEN '".$_GET['date_from']."' AND '".$_GET['date_to']."' AND ssh_p_dialysis.admission_type = '0' ";
                                        ...
                                        foreach($fetch_data_ex as $row){ 
                                            ...
                                            <tr id="<?php echo $row['pd_id'] ?>">
                                                ...
                                                if ($row['injection'] == '1') {
                                                     echo "<td>".$row['Name']."<br>".$row['Paid']."</td><td>Yes</td><td>".$row['phone']."</td><td>".intval($dicount)."</td><td>".$row['date']."</td>";
                                                }elseif($row['injection'] == '0'){
                                                    echo "<td>".$row['Name']."<br>".$row['Paid']."</td><td>No</td><td>".$row['phone']."</td><td>".intval($dicount)."</td><td>".$row['date']."</td>"; 
                                                }else{
                                                    echo "<td>".$row['Name']."<br>".$row['Paid']."</td><td>Unknown</td><td>".$row['phone']."</td><td>".intval($dicount)."</td><td>".$row['date']."</td>";
                                                }
                                                ?>
