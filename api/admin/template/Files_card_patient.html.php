...
<div class="col-xl-12  col-lg-12">
    <div class="card">
        <style type="text/css">
            ...
            <div class="text-center col-md-12">  
                ...
                <form action="" method="get" enctype="multipart/form-data">
                      ...
                      <input type="text" class="form-control ml-2"  name="visitor_id"  style="width: 150px;float: left;" required placeholder="Visitor-ID">
                     

                    <input type="submit" class="btn btn-success  ml-2"  name="search_visitor_id" value="Search"  style="float: left;height: 36px;">
        ...
        <?php  } elseif (isset($_GET['search_visitor_id'])) { ?>
                            <style>
                                #myInput_view {
                                  background-image: url('/css/searchicon.png');
                                  background-position: 10px 10px;
                        ...
                        <tbody>
                            ...
                            $total_ex = 0;
                            $sr_no = 1;
                            $fetch_data = "SELECT * FROM ssh_p_indoor JOIN ssh_p_reg ON ssh_p_indoor.P_ID = ssh_p_reg.P_ID LEFT JOIN ssh_cases_indoor ON  ssh_p_indoor.S_ID = ssh_cases_indoor.S_ID where ssh_p_indoor.exit_date != '0000-00-00' AND ssh_p_indoor.admition_type = '1' AND ssh_p_indoor.visitor_id = '".$_GET['visitor_id']."'  ORDER BY ssh_p_indoor.file_date DESC  ";
                            $fetch_data_ex = mysqli_query($con,$fetch_data);
                            foreach($fetch_data_ex as $row){ 
                                $date1 = $row['file_date'];
                                $date2 = date('Y-m-d');
                            ...
                            <?php
                            $total_ex = 0;
                            $fetch_data = "SELECT * FROM ssh_p_dialysis JOIN ssh_p_reg ON ssh_p_dialysis.P_ID = ssh_p_reg.P_ID where ssh_p_dialysis.admission_type = '1' AND ssh_p_dialysis.visitor_id =  '".$_GET['visitor_id']."'  ORDER BY ssh_p_dialysis.file_date DESC ";
                            $fetch_data_ex = mysqli_query($con,$fetch_data);
                            foreach($fetch_data_ex as $row){ 
                                $date1 = $row['file_date'];
                                $date2 = date('Y-m-d');
