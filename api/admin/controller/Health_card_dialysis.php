...
if (isset($_POST['psubmit'])) {
	...
	if (!empty($pat_id)){
		...
		if ($update_data_ex) {
			...
			$insert_data = "INSERT INTO `ssh_p_dialysis`(`visitor_id`, `P_ID`,`injection`,`Paid`, `date`, `admission_type`) VALUES ('".$visitor_id."','".$pat_id."','".$injection."','".$Paid."','".date('Y-m-d')."','1')";
			...
			$insert1_data = "INSERT INTO ssh_p_reg(Name,Age,Phone,Gender) VALUES ('".$pat_Name."','".$pat_Age."','".$pat_Phone."','".$pat_gender."')";
				...
				if ($insert1_data_ex) {
					...
					$insert_data = "INSERT INTO `ssh_p_dialysis`(`visitor_id`, `P_ID`,`injection`,`Paid`, `date`, `admission_type`) VALUES ('".$visitor_id."','".$injection."','".$last_p_id."','".$Paid."','".date('Y-m-d')."','1')";
						$insert_data_ex = mysqli_query($con,$insert_data);
						if ($insert_data_ex) {
							$last_id = $con->insert_id;
							?>
