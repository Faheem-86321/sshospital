...
if (isset($_POST['psubmit'])) {
	...
	if (!empty($pat_id)){
		...
		if ($update_data_ex) {
				...
				$insert_data = "INSERT INTO `ssh_p_indoor`(`visitor_id`, `P_ID`, `S_ID`, `Paid`, `admit_date`, `admition_type`, `room_id`, `close`) VALUES ('".$visitor_id."','".$pat_id."','".$case_id."','".$Paid."','".date('Y-m-d')."','0','".$room_id."','1')";
		...
		$insert1_data = "INSERT INTO ssh_p_reg(Name,Age,Phone,Gender) VALUES ('".$pat_Name."','".$pat_Age."','".$pat_Phone."','".$pat_gender."')";
			...
			if ($insert1_data_ex) {
				...
				$insert_data = "INSERT INTO `ssh_p_indoor`(`visitor_id`, `P_ID`, `S_ID`, `Paid`, `admit_date`, `admition_type`, `room_id`, `close`) VALUES ('".$visitor_id."','".$last_p_id."','".$case_id."','".$Paid."','".date('Y-m-d')."','0','".$room_id."','1')";
				$insert_data_ex = mysqli_query($con,$insert_data);
					if ($insert_data_ex) {
						$last_id = $con->insert_id;
					$doc_id = $_POST['doc_id'];
